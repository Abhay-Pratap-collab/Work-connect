...
export default function ExpertIconEdit({ refresh, setRefresh, rowData, openStatus, setOpenStatus }) {
  const classes = useStyle();
  const [photograph, setPhoto] = useState({ file: 'india-gate.png', bytes: '' })
  
  const handleImage = (e) => {
    setPhoto({ file: URL.createObjectURL(e.target.files[0]), bytes: e.target.files[0] })
...
//   const [cityId, setcityId] = useState('')
  ...
  const handleCancel = () => {
    setPhoto({ file: `${serverURL}/images/${rowData?.photograph}`, bytes: '' })
  ...
  const handleSubmit = async () => {
    ...
    var res = await postData('experts/edit_photo', body)
    setLoading(false);
    if (res.status) {
      Swal.fire({
        // position: "top-end",
        ...
        <div className={classes.boxEdit}>
          <div className={classes.heading}>
            <div className={classes.headingGroupStyle}>
              ...
              <span className={classes.haedingText}>Edit photograp</span>
            </div>
          </div>
          <div style={{ margin: 10, width: "96.5%" }}>
            <Grid spacing={2} container>
              <Grid size={12} className={classes.centerStyle}>
                <img src={`${photograph.file}`} style={{ width: 300 }} />
              </Grid>

              <Grid size={6} className={classes.centerStyle}>
                {saveBTnStatus ? saveCancelBtn() :
                  <Button component="label" variant="contained" startIcon={<CloudUploadIcon />}>Upload New Image
