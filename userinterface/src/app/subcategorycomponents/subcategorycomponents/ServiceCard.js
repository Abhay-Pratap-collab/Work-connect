  ...
  return (
    <div className={styles.serviceBox} >
      <div className={styles.priceBox} >
        ...
        {data.offer == "" ? <p style={{ fontSize: '14px', fontWeight: '700' }} >Starts at ₹{data.amount}</p> : <div style={{ fontSize: '12px', display: 'flex', alignItems: 'center', gap: '10px' }}><h4>{`₹${data.offer}`}</h4><s>{`₹${data.amount}`}</s><p>{data.time_services}</p></div>}
